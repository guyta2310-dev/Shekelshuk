const SHEKEL = "\u20AA";
const STORAGE_KEY = "shekelshuk-state-v2";
...
function loadState() {
  ...
  try {
    ...
    const loadedState = {
      ...
      markets: Array.isArray(saved.markets) ? saved.markets : defaults.markets,
      tradeBudgets: {
        ...defaults.tradeBudgets,
        ...(saved.tradeBudgets || {}),
      },
      transactions: Array.isArray(saved.transactions) && saved.transactions.length ? saved.transactions : defaults.transactions,
      admin: {
        ...defaults.admin,
        ...(saved.admin || {}),
      },
      casino: {
        dice: {
          ...defaults.casino.dice,
          ...(saved.casino?.dice || {}),
          rolling: false,
        },
        plinko: {
          ...defaults.casino.plinko,
          ...(saved.casino?.plinko || {}),
          dropping: false,
          currentStep: 0,
          path: [],
          slotIndex: saved.casino?.plinko?.slotIndex ?? null,
        },
        blackjack: {
          ...defaults.casino.blackjack,
          ...(saved.casino?.blackjack || {}),
          deck: Array.isArray(saved.casino?.blackjack?.deck) ? saved.casino.blackjack.deck : [],
        },
      },
    };

    if (!loadedState.users.some((user) => user.id === loadedState.currentUserId)) {
      loadedState.currentUserId = null;
    }
...
function renderHeader() {
  if (!refs.headerBalance || !refs.headerUser) return;
...
function renderCasino() {
  if (!refs.casinoGrid) return;
  const user = getCurrentUser();
  const wallet = user ? formatShekels(user.balance) : "Register first";
  const blackjack = state.casino.blackjack;
  const dealerTotal = !blackjack.dealer.length ? "--" : blackjack.revealDealer ? getHandValue(blackjack.dealer) : "Hidden";
  const playerTotal = !blackjack.player.length ? "--" : getHandValue(blackjack.player);
  refs.casinoGrid.innerHTML = `
    <article class="casino-card dice-card">
      <div class="card-head">
        <div>
      ...
      <div class="casino-stage dice-stage ${state.casino.dice.rolling ? "is-rolling" : ""}" data-dice-stage>
        <div class="dice-core">
          <span class="small-label">Live result</span>
          <div class="dice-number" data-dice-number>${state.casino.dice.display}</div>
          <p class="stage-copy" data-dice-message>${escapeHtml(state.casino.dice.message)}</p>
        </div>
      </div>
      ...
      <div class="recent-rolls">
        ${state.casino.dice.recent
          .map((roll, index) => `<span class="roll-chip ${index === 0 ? "latest" : ""}">${roll}</span>`)
          .join("")}
      </div>
      <div class="bank-pill">
    ...
    <article class="casino-card plinko-card">
      <div class="card-head">
        <div>
      ...
      <div class="bank-pill note-pill">
        <span>Latest note</span>
        <strong>${escapeHtml(state.casino.plinko.message)}</strong>
      </div>
    </article>

    <article class="casino-card blackjack-card">
      <div class="card-head">
        <div>
      ...
      <label class="field">
        <span>Bet amount</span>
        <input id="blackjackBet" type="number" min="10" step="5" value="${blackjack.bet}" />
      </label>
      <div class="hands table-stage">
        <div class="card-lane">
          <div class="lane-head">
            <span>Dealer</span>
            <strong class="hand-total">${dealerTotal}</strong>
          </div>
          <div class="cards">${renderCards(blackjack.dealer, !blackjack.revealDealer)}</div>
        </div>
        <div class="card-lane">
          <div class="lane-head">
            <span>Player</span>
            <strong class="hand-total">${playerTotal}</strong>
          </div>
          <div class="cards">${renderCards(blackjack.player, false)}</div>
        </div>
      </div>
      <p class="stage-copy">${escapeHtml(blackjack.message)}</p>
      <div class="blackjack-actions">
        <button class="primary-button" data-action="blackjack-start" ${!user || blackjack.inRound ? "disabled" : ""}>Deal</button>
        <button class="secondary-button" data-action="blackjack-hit" ${blackjack.inRound ? "" : "disabled"}>Hit</button>
        <button class="ghost-button" data-action="blackjack-stand" ${blackjack.inRound ? "" : "disabled"}>Stand</button>
      </div>
      <div class="bank-pill">
...
function renderPlinkoBoard() {
  ...
  return `
    <div class="plinko-board ${board.dropping ? "is-dropping" : ""}" data-plinko-board>
      <div class="peg-field">
      ...
      <div class="plinko-ball ${board.dropping ? "is-live" : ""}" data-plinko-ball style="left: ${currentX}%; top: ${currentY}%"></div>
      <div class="plinko-slots">
        ${board.multipliers
          .map(
            (multiplier, index) => `
              <div class="slot ${index === board.slotIndex ? "active" : ""}" data-plinko-slot="${index}">
...
function renderCards(cards, hideFirstCard) {
  ...
  return cards
    .map((card, index) => {
      ...
      const [rank, suit] = card.label.split("-");
      return `
        <div class="play-card" data-suit="${escapeHtml(suit)}">
...
function playDice() {
  ...
  state.casino.dice.message = `Rolling ${formatShekels(bet)} through the chamber...`;
  renderHeader();
  renderMetrics();
  renderCasino();

  const diceNumberEl = refs.casinoGrid.querySelector("[data-dice-number]");
  ...
  const animateDice = (now) => {
    ...
    const cadence = 45 + elapsed * 0.16;

    if (elapsed >= lastFlipAt) {
      state.casino.dice.display = randomInt(1, 6);
      if (diceNumberEl) {
        diceNumberEl.textContent = String(state.casino.dice.display);
      }
      lastFlipAt += cadence;
    }
...
function playPlinko() {
  ...
  const ball = refs.casinoGrid.querySelector("[data-plinko-ball]");
  const slots = Array.from(refs.casinoGrid.querySelectorAll("[data-plinko-slot]"));
  const points = path.map((slot, index) => getPlinkoBallPoint(slot, index));
  const startTime = performance.now();

  cancelAnimationFrame(plinkoAnimationFrame);
  const animateDrop = (now) => {
    const progress = clamp((now - startTime) / 1400, 0, 1);
    ...
    const toPoint = points[segmentIndex + 1] || points[segmentIndex];
    const liveX = lerp(fromPoint.x, toPoint.x, localProgress);
    const liveY = lerp(fromPoint.y, toPoint.y, localProgress);

    if (ball) {
      ball.style.left = `${liveX}%`;
      ball.style.top = `${liveY}%`;
      ball.style.transform = `translate(-50%, -50%) scale(${1 + Math.sin(progress * Math.PI * 6) * 0.06})`;
    }

    const liveSlot = progress > 0.78 ? path[Math.min(path.length - 1, Math.floor(progress * path.length))] : null;
    slots.forEach((slotEl, index) => {
      slotEl.classList.toggle("glow", liveSlot === index);
    });

    if (progress < 1) {
      plinkoAnimationFrame = requestAnimationFrame(animateDrop);
      return;
    }

    const finalSlot = path[path.length - 1];
    const multiplier = game.multipliers[finalSlot];
    const payout = roundMoney(bet * multiplier);

    game.currentStep = path.length - 1;
    game.slotIndex = finalSlot;
    game.dropping = false;
    game.message = `Slot ${finalSlot + 1} landed at ${formatMultiplier(multiplier)} for ${formatShekels(payout)}.`;
...
function getPlinkoBallPoint(slot, step) {
  return {
    x: 13 + slot * 9.25,
    y: step === 0 ? 8 : 10 + step * 9.5,
  };
}

function getPlinkoBallX(game) {
  const slot = game.path[game.currentStep] ?? 4;
  return getPlinkoBallPoint(slot, game.currentStep).x;
}

function getPlinkoBallY(game) {
  const slot = game.path[game.currentStep] ?? 4;
  return getPlinkoBallPoint(slot, game.currentStep).y;
}
